...
export default function StarRatingG() {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      const { data, error } = await supabase.from('profiles').select('starsGajuz');
  ...
  async function getAvg() {
    ...
    const allRatings = data.map(item => item.starsGajuz).filter(starsGajuz => Number.isInteger(starsGajuz));
...
const [value, setValue] = useState(null);
  ...
  async function UpdateRating() {
   // console.log("updated to:" + value);
    const {data, error} = await supabase.from('profiles').update({ starsGajuz: value }).eq('id', session?.user.id).select() //session.user.id
    
     if(data){
       setDajHviezdy(data[0].starsGajuz) 
       console.log("loggedUserUpdated:"+data[0].starsGajuz)
  ...
  useEffect(() => {
    const fetchValue = async () => {
      const { data } = await supabase.from('profiles').select('starsGajuz').eq('id', session?.user.id)
